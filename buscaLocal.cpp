...
BuscaLocal::BuscaLocal(Grafo* grafo, std::string buscaLocal, std::string buscaLocalProx) {
    matriz = grafo->getMatrizAdjacencia();
    this->buscaLocal = buscaLocal;
    this->buscaLocalProx = buscaLocalProx;
}
...
void BuscaLocal::realizaBuscaLocal(Grafo *&grafo, std::string &busca, std::vector<Grupo> solucao, 
                                   bool &melhorou) {
    if (busca == "insertionImp") {
        insercaoAlgoritmo(solucao, melhorou);
    }
    else if (busca == "swapImp") {
        swapAlgoritmo(melhorou);
    }
    else if (busca == "threeChainImp") {
        swapEmCadeiaAlgoritmo(melhorou);
    }
}

std::vector<Grupo> BuscaLocal::busca(Grafo* grafo, std::vector<Grupo> solucao) {
    std::vector<Grupo> solucaoFinal = solucao;
    criaVariaveis(grafo, solucao);
    bool melhorou = true;
    while (melhorou) {
        melhorou = false;
        realizaBuscaLocal(grafo, buscaLocal, solucaoFinal, melhorou);
        realizaBuscaLocal(grafo, buscaLocalProx, solucaoFinal, melhorou);
